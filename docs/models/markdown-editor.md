...
- Settings 提供 `Appearance` 面板，用于配置应用语言偏好、深浅色主题、界面密度和动效偏好；语言设置保存到本地 `app_settings`，并通过 `app/frontend/src/i18n.ts` 驱动已接入界面的英文 / 简体中文文案切换。
- Settings 提供 `Typography` 面板，用于调整编辑器显示字号。默认字号为 `16px`，可在 `13px` 到 `48px` 范围内调整；设置时单字预览会即时跟随变化，正文段落、标题、列表、引用、代码、表格、Mermaid、数学公式、渲染视图、源码视图和分屏编辑器都会使用同一基础字号，并保存到本地 `app_settings`。
...
- 打开的文件没有任何内容时，渲染编辑器首段必须显示占位提示：`Start writing in Markdown, or type / to use commands.`，明确引导用户输入 `/` 使用指令菜单。
