...
- 编辑器支持唤起浮动 Agent Palette：
  - 当编辑器内存在文字选区时，浮层会读取当前浏览器选区最后一行的矩形，并定位在该行正下方；
  - 当没有有效选区时，浮层出现在编辑区中上方的居中位置；
  - 如果选区距离窗口底部过近，编辑滚动区会临时增加底部安全空间，并向下滚动出足够空间放置 prompt 框。
- 当前原型监听 `Fn` 键；由于不同系统和 Electron 版本可能不会向渲染进程暴露独立 `Fn` 事件，同时提供 `Cmd/Ctrl + J` 作为开发验收兜底快捷键。
- Prompt 框包含：
