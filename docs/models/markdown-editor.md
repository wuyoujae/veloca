...
- Tables now use a dedicated interaction layer: `Enter` inserts an in-cell line break, `Shift+Enter` inserts a new body row below the current row and keeps the same column focused, holding `ArrowLeft` or `ArrowRight` while pressing `Enter` inserts a new column on the current column's left or right side and focuses the new cell in the same row, and arrowing above or below the table boundary exits into a real paragraph block.
