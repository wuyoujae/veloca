...
- Tables now use a dedicated interaction layer: `Enter` inserts an in-cell line break, `Shift+Enter` inserts a new body row below the current row and keeps the same column focused, `Shift + ArrowLeft` and `Shift + ArrowRight` insert columns to the current column's left or right side, `Shift + ArrowDown` inserts a row below, `Shift + ArrowUp` inserts a row above only when the current row is not the header, and arrowing above or below the table boundary exits into a real paragraph block.
- When the text cursor is inside a table, Veloca shows Notion-style block controls outside the content gutter on the table's left side. The first control opens a `5 x 10` resize matrix that directly resizes the current table while preserving overlapping cell content, and the second control opens a menu for inserting rows or columns with the same keyboard shortcuts shown in the UI.
