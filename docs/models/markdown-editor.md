...
- 编辑器支持通过独立 Electron 子窗口唤起 Agent Palette，避免主编辑器窗口尺寸或滚动区域裁切对话框：
  - 当编辑器内存在文字选区时，主窗口会读取当前浏览器选区最后一行的矩形，并通过 IPC 将屏幕坐标交给主进程；
  - 主进程使用独立 `BrowserWindow` 加载 `agent.html`，根据选区屏幕坐标放置窗口，并按当前显示器工作区边界进行安全收敛；
  - 当没有有效选区时，Agent 子窗口会出现在主窗口中上方的居中位置；
  - 如果选区距离窗口底部过近，编辑滚动区仍会临时增加底部安全空间，并向下滚动出足够空间，让 Agent 尽量贴近选中文字下方。
- 当前原型在主进程监听 Agent 快捷键：
  - `before-input-event` 会尝试捕获 `Fn` 键和 `Cmd/Ctrl + J`；
  - `globalShortcut` 会注册 `Cmd/Ctrl + J`，并尝试注册 `Fn`；
  - 由于 macOS/Electron 不一定会把单独 `Fn` 作为标准按键或 accelerator 暴露出来，`Cmd/Ctrl + J` 是稳定的开发验收兜底快捷键。
- Prompt 框包含：
  ...
  - 主按钮左侧的麦克风按钮用于“语音转文字”模式，prompt 框保持文字输入状态，按钮展示呼吸动画并在识别完成后把文字插入当前光标位置；
