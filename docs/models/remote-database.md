...
- `POST /v1/projects` 创建项目，传入 `name`、`organization_slug`、`db_pass` 和 `region`。
...
2. 用户输入 Supabase PAT、organization slug、region 和 database password。
3. 用户点击 `Create / Connect Veloca Project`。
4. Veloca 主进程加密保存敏感信息。
5. Veloca 校验组织并查找同名项目；存在则复用，不存在则创建。
6. Veloca 等待项目 ready，直连 Postgres 初始化基础表。
7. Veloca 保存项目 ref、URL、host 和 API key 状态。
