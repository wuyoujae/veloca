...
- `GET /v1/projects/available-regions?organization_slug={slug}` 读取该组织当前可用的 smart group 和 specific region，前端只允许用户从返回结果中选择。
...
3. Veloca 调用 Supabase available regions API 并在 Remote 面板显示地区下拉选项。
4. 用户选择一个 Supabase 返回的 region。
5. 用户点击 `Create / Connect Veloca Project`。
6. Veloca 主进程加密保存敏感信息。
7. Veloca 校验组织并查找同名项目；存在则复用，不存在则创建。
8. Veloca 等待项目 ready，直连 Postgres 初始化基础表。
9. Veloca 保存项目 ref、URL、host 和 API key 状态。
